...
Find the sample Voiceflow to Rasa connector [here](https://github.com/voiceflow/rasa-export-script-demo). **This is a starter template which we expect will be evolved based on customer's conversation design system on the Voiceflow platform.**

# Overview

This repo contains a set of functions that allow for exporting a Rasa assistant from a Voiceflow project. The process involves reading the project data, parsing the data, and then generating the domain and stories files.
...
This will produce a .zip file with the same name (ex. `project.vf` -> `project.zip`). This file contains a the `domain.yml`, `nlu.yml`, and `stories.yml` files for use with Rasa.

## Files

### `index.ts`

This file is the main entry point for the script. It reads the project data, parses it, and generates the Rasa assistant.
...
### `main()`
 ...
 The main function of the project. It reads the JSON file, converts the Voiceflow intents and entities into their Rasa equivalents, and then creates a Rasa assistant using these converted intents and entities.
...
This function generates Rasa utterances for a given Voiceflow intent and adds it to the nlu.yml file
