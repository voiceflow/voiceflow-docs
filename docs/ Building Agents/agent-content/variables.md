...
Variables are set on a per-user basis, [based on the `user_id` variable](https://docs.voiceflow.com/docs/variables#built-in-variables). This means that if your agent is having a conversation with ten different users at the same time, the values of their variables are not shared between users.
