---
title: Custom deployment channel (API)
excerpt: Use the Dialog API to deploy agents to custom channels.
deprecated: false
hidden: false
metadata:
  title: ''
...
You can use the Dialog API to integrate your <<glossary:agent>> with any custom channel or interface (voice, chat or multimodal). Any Voiceflow <<glossary:agent>> type works with a Custom Channel deployment as it uses the Dialog Manager API, which is available across every project type.
...
1. Open the <<glossary:agent>> project you are using
  ...
  "images": [
    {
      ...
      "align": "center",
      "caption": "Above you can see the Integrations section within an agent project"
    }
  ]
}
[/block]
