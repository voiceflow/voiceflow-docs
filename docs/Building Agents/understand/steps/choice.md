...
_**Tip:** In general, anything you can do with a choice step can be done with a button step (including attaching intents to paths), except the button step also has a visual users can click on, so it's often better to just use buttons._
...
_**Note**: In its default state (ie. when you drag a Choice step from the sidebar) it will appear and function as to 'Listening for an intent' ; if you click Add Choice and click the settings configuration you can limit this function._
...
_Note: You can modify a path's linked intent without deleting the path's connection point._
...
_**Tip**: Learn more about Actions in-depth and in detail here._
...
_**Tip**: You will notice you can also preview the required entities associated with the Intent and its utterances._
...
_**Tip**: Under the Required entities section, this can also suggest entities populated from your utterances to include the option as quick-buttons, to serve as a function to add it to your required entities._
