...
Behind the scenes, Voiceflow uses Svix to send events to your webhook URL. If an error occurs when attempting to send data to your webhook URL, we'll periodically retry sending the event to you [based on their retry schedule](https://docs.svix.com/retries) .
