...
next:
  description: ''
---
Instead of using the default NLP service (Microsoft LUIS) provided by Voiceflow, it's easy to use a preferred custom NLP service. When making an API request, instead of using a `TextRequest` where the raw string of the user input is provided, use an `IntentRequest` and the default Voiceflow NLP is skipped.
...
If the intent on Voiceflow has required entities (i.e `{pizza_size}` for `order_pizza_intent`) and it is not filled in the initial request, the entity will be prompted for and you can send a subsequent request with the same intent and the `entities` and Voiceflow will be able to automatically merge the initial entities with the new entities.
